...
* **April 2020** The first attempt at adding a new compiler driver for Flang is
  [posted](https://reviews.llvm.org/D79092) for review. This used the name
  `flang-tmp`. This was later abandoned in favour of a different approach.
...
Flang became the the first project to use Clang's compiler driver library
(`clangDriver`) to implement its own compiler driver.

Doing so:
* Gave Flang support for a large number of targets and operating systems.
...
behave for Fortran (as opposed to the C and C++ that Clang compiles).
