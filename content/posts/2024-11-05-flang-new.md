...
is today) in use. Classic Flang's binary name was `flang`, just like the folder
F18 now occupied in the LLVM monorepo.
...
* **July 2022** Valentin Clement (NVIDIA) [announces](https://discourse.llvm.org/t/nvidia-transition-from-fir-dev/61947/5)
...
If you want more detail please refer to that document.

The term "Driver" in a compiler context means the part of the compiler that decides
...
The "compiler driver" in this case is `flang` and the "frontend driver" is
`flang -fc1` (they are in the same binary).

The main reason they are split into 2 is so that the "compiler driver" can have
option names that do not change over time and the "frontend driver" can change
its option names as suits the developers of the compiler.
...
This created a problem for implementing Flang's compiler driver. This many
details would take a long time to get right.

Luckily for them, by this time Flang was in the LLVM Project, alongside Clang.
...
Thus Flang became the the first project to use Clang's compiler driver
library (`clangDriver`) to implement its own compiler driver.

Doing so:
* Gave Flang support for a myriad of diverse targets and operating systems.
* Let Flang control any linker or assembler that Clang could.
* Made it possible to add any Clang option to Flang.

Note that not all Clang options have been, or will be added to Flang. This is so
that when a new option is proposed, there is a chance to consider how it should
behave for Fortran (as opposed to the C and C++ Clang compiles).
