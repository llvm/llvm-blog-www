...
OpenMP is a standardised open API for adding parallelisation to C, C++ and
Fortran programs. Programmers use compiler directives to mark parts of their
source code, the compiler interprets these and produces calls to a runtime
library that distributes work as instructed.
...
```
(taken from ["OpenMP Application Programming Interface Examples"](https://www.openmp.org/wp-content/uploads/openmp-examples-4.5.0.pdf), try it on [Compiler Explorer](https://godbolt.org/z/chjzs3o6r))

**Note:** Fortran arrays are [one-based](https://fortran-lang.org/en/learn/quickstart/arrays_strings/) by default, so the first element is at index 1. This example reads the previous entry, so it starts with `I` as 2.

`!$OMP PARALLEL DO` is a directive, in the form of a Fortran comment, which
start with `!`. `PARALLEL DO` starts a parallel "region" which covers the
...
Overall very similar to the Fortran.

LLVM IR does not know anything about OpenMP specifically, so Clang uses a
library called `OpenMPIRBuilder` to convert these directives into LLVM IR.
...
Notice that `simple` no longer does the work. Instead it uses `__kmpc_fork_call`
to call another version of `simple`, which calls yet another version. `__kmpc_fork_call`
is part of the OpenMP [runtime library](https://github.com/llvm/llvm-project/tree/a23260087db14032094d62dcf8be9be6f4e3d788/openmp). The final
`.omp_outlined_debug__` version of `simple` is where the work happens.

This example shows that LLVM IR has no special knowledge of what OpenMP is.
There are labels like `omp.precond.then`, but this is just a name. It is
not any different to a normal label.
...
This dialect would understand OpenMP specifically, unlike the LLVM IR you 
get from Clang. 

Back to the Fortran version of the OpenMP example. This is the MLIR
produced if you do not enable OpenMP:
...
Notice that the `DO` loop has been converted into `for.do_loop`. Look what
happens when you enable OpenMP:
...
This is a much more literal translation of the `parallel` directive than Clang
would give you.

Since the `omp` dialect is specifically made for OpenMP, it can represent
it in a much more natural manner. This makes understnding the code and writing
optimisations much easier.

Of course Flang needs to produce LLVM IR eventually, and to do this it
reused Clang's `OpenMPIRBuilder` library.

```mlir
define void @simple_(<...> {
entry:
  call void (ptr, i32, ptr, ...) @__kmpc_fork_call( <...> @simple_..omp_par <...>
  ret void
}
...
this is not unexpected. Given that the source language is different and Flang's
IR also goes through MLIR's compiler passes.

## ClangIR and the Future

It is surprising that a compiler for a language as old as Fotran got ahead of
the most known LLVM based compiler, Clang, when it comes to adopting MLIR.

This is due to timing, MLIR is a recent invention and Clang has existed since
long before MLIR arrived. Clang also has a legacy to protect, so it was unlikely
to migrate to an unproven technology immediately.

That said, the [ClangIR](https://llvm.github.io/clangir/) project aims to
change Clang to use a new MLIR dialect "Clang Intermediate Representation" ("CIR"). Much like Flang's HLFIR/FIR, ClangIR will convert C and C++ into the
CIR dialect and later lower it into LLVM IR.
