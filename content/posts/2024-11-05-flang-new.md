...
the Flang binary was not `flang` (like `clang`), it was `flang-new`.
...
will cover the almost 10 year journey of Flang, right from first concepts.
Through multiple rewrites, the adoption of LLVM frameworks like MLIR and the
entry of Flang into the LLVM Project.
...
Instead they could write code that looked like their formulas. Something you would
expect today but at the time was a revolution. This focus led to Fortran being used
...
* Develop Flang under a permissive license like that of [LLVM](https://llvm.org/LICENSE.txt),
...
Though the first signs of the current "LLVM Flang" did not appear until 2017, the
history of the Portland Group is important context. Particularly their previous
...
* **May 2017** The first source code release of Classic Flang as a separate
  respository, outside of the LLVM Project. Comprised of a frontend contributed
...
* **April 2018** Steve Scalpone (NVIDIA) [announces](https://www.youtube.com/watch?v=sFVRQDgKihY)
  at Euro LLVM 2018 that the frontend of Classic Flang will be rewritten to address
...
unique features of your language, optimise that, then convert the result into LLVM IR.
...
the project. It became its own public branch `fir-dev`, that was later upstreamed
...
(HLFIR). This works at a higher level than FIR. A big target of HLFIR
...
```
For example, the "dummy arguments" (the name for the
...
The next step is to convert the HLFIR into FIR and the `arith` (arithmetic) dialect:
```mlir
module attributes {<...>} {
...
`llvm`, that both compilers already heavily relied on.
...
As you will see in the next sections, Flang has diverged from Clang for other
...
This made Flang able to handle all the targets and tools that Clang could. Without
