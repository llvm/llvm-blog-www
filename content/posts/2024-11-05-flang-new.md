...
. This means it cannot optimise based on, for example, the fact that
the program is using Fortran arrays.
...
We all know naming is hard but Eric nailed that aspect:

> FIR was a pun of sorts. Fortran IR and meant to be evocative of the trees
> (Abstract Syntax Trees).

I am not going to go into detail about this early FIR, because it quickly
...
One advantage of the rewriting of Flang between Classic Flang and LLVM Flang is
that it allowed LLVM Flang to adopt new features from the LLVM Project. The most
notable being MLIR.
...
The problem MLIR addresses is the same one that Eric Schweitz was addressing
with the early versions of FIR. It is difficult to map high level details
of programming languages into LLVM IR.
...
languages, and Fortran array optimisations were an early cited use case for MLIR.

This lead languages such as Swift, Rust and Julia to create their own IRs
which include information relevant for their own optimisations.
...
In Eric Schweitz's experience, MLIR lived up to the hype:
...
Flang did not stop there, later adding another MLIR dialect
...
which duplicates `fir.array`, unless HLFIR wanted to handle it in a unique way.

The next step is to convert the HLFIR parts into FIR and MLIR's `arith`
(arithmetic) dialect:
...
If Flang were to do this later on LLVM IR it would be much more difficult. Either
the information would be lost or incomplete, or we would be at a stage in the
pipeline where we cannot assume that we started with a specific source language.
