...
OpenMP is a standardised API for adding parallelisation to C, C++ and
Fortran programs. Programmers mark parts of their code with directives.
...
```
(taken from ["OpenMP Application Programming Interface Examples"](https://www.openmp.org/wp-content/uploads/openmp-examples-4.5.0.pdf), [Compiler Explorer](https://godbolt.org/z/chjzs3o6r))
...
`simple` no longer does the work of the program. Instead it uses
`__kmpc_fork_call` to call another version of `simple`, which calls yet another
version. `__kmpc_fork_call` is part of the OpenMP
[runtime library](https://github.com/llvm/llvm-project/tree/a23260087db14032094d62dcf8be9be6f4e3d788/openmp). The final
`.omp_outlined_debug__` version of `simple` is where the work actually happens.

It includes labels like `omp.inner.for.body` but these are not specific to OpenMP.
They are just normal LLVM IR labels.
...
Notice that the `DO` loop has been converted into `for.do_loop`. Now enable
...
this is not unexpected. The original source language is different and Flang's
