...
define internal void @simple(<...>) @simple <...> (.omp_outlined) <...> {
entry:
<...>
...
`__kmpc_fork_call` to call another version of `simple`. `__kmpc_fork_call` is
part of the OpenMP [runtime library](https://github.com/llvm/llvm-project/tree/a23260087db14032094d62dcf8be9be6f4e3d788/openmp). This second `.omp_outlined`
version of the function is where the work is actually done, and the compiler
has rearranged the code to facilitate that (you can see the full output on
Compiler Explorer).

This LLVM IR includes labels like `omp.loop.exit.i` but these are not
specific to OpenMP. They are just normal LLVM IR labels whose name includes
`omp`.
