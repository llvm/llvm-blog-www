...
source code, the compiler interprets these and then the work is handed off to
a runtime library.

This is what an OpenMP enabled program looks like in Fortran:
```Fortran
...
```
(taken from ["OpenMP Application Programming Interface Examples"](https://www.openmp.org/wp-content/uploads/openmp-examples-4.5.0.pdf), try it on [Compiler Explorer](https://godbolt.org/z/EKhGfGfa9))

**Note:** Fortran arrays are [one-based](https://fortran-lang.org/en/learn/quickstart/arrays_strings/) by default. This means that the first element is
at index 1.

`!$OMP PARALLEL DO` is a directive, in the form of a Fortran comment, which
all start with `!`. `PARALLEL DO` starts a parallel region, which encloses the
`DO` loop and ends at `!$OMP END PARALLEL DO`.

This tells the compiler that the work in the `DO` loop should be shared amongst
all the threads available to the program (this number is controlled by
settings in its environment).

Clang has [supported OpenMP](https://blog.llvm.org/2015/05/openmp-support_22.html)
since before even Classic Flang was first announced in late 2015. The equivalent
C++ program is:
```C++
...
```
(again from the OpenMP examples document, [Compiler Explorer](https://godbolt.org/z/eca6vjbqe))

LLVM IR does not know anything about OpenMP specifically, so Clang uses a
library called `OpenMPIRBuilder` to convert these pragmas into LLVM IR that
contains calls to the OpenMP runtime.

TODO: IR example goes here.
